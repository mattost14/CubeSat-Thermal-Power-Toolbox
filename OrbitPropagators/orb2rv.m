...
%------------------------------------------------------------------       %
function [r,v] = orb2rv(a,e,i,O,o,nu,truLon,argLat,lonPer,mu)
if ~exist('mu','var') 
    mu = 4.9048695e12/(1e3)^3;     % Moon Gravitational Constant, unit: km^3/s^2
%     mu = 398600.4418; 
end

%Make all inputs consistent w/ dimensions
a = a(:); e = e(:); i = i(:); O = O(:); o = o(:); nu = nu(:);
% Conversion from semi-major axis to semilatus rectum
r_plus = a*(1+e); r_minus = a*(1-e);
p = (.5*(1/r_plus + 1/r_minus))^-1;
